...
public class GridManagerMult : MonoBehaviourPunCallbacks
{
    ...
    public void CreateGrid(Vector3 position)
    {
        ...
        Arfeathered = GameObject.FindObjectsOfType<ARPlane>();
        foreach (ARPlane plane in Arfeathered)
        {
            Destroy(plane.gameObject); // Remove the plane's GameObject
        }
        // Synchronize the grid with other players
        photonView.RPC("SyncGrid", RpcTarget.OthersBuffered, position);
    }

    [PunRPC]
    public void SyncGrid(Vector3 position)
    {
        // Create the grid for the other player
        CreateGrid(position);
    }
    ...
    private void PlaceShip(int startX, int startY, GameObject shipPrefab)
    {
        ...
        // Synchronize ship placement with other players
        photonView.RPC("SyncShipPlacement", RpcTarget.OthersBuffered, startX, startY, shipPrefab.name);
    }

    [PunRPC]
    public void SyncShipPlacement(int startX, int startY, string shipName)
    {
        // Find the ship prefab by name
        GameObject shipPrefab = System.Array.Find(shipPrefabs, ship => ship.name == shipName);
        if (shipPrefab != null)
        {
            PlaceShip(startX, startY, shipPrefab);
        }
